...
%algorithm to match cells (or anythign else) between successive frames. 
...
% different time point. Each row in peaks should be a different object
...
% columns). The dummy index can be initialized to -1 in every row. It will
% be filled in by match frames so that its value gives the row where the
% data on teh same cell can be found in the next frame. 
...
%and column 4 should be initialize with -1 in all rows. 
...
%correspodence. You can use a value of L = 1. Verify that it works by writing code to find the mean difference between the
...
% point. Rerun Match frames and look at the resulting match. What happened? 
...
% into two about half way through. Use any method to segment the movie and
% fill a cell array peaks with 5 columns of data as above - 
% the x,y coordinates of the cells, the area of the cells. -1 in all rows.
% The mean fluorescence intensity of the cells. Run MatchFrames in a loop
% to fill in the fourth column peaks for every frame. 

% Part 4. The Tracking algorithm works by minimizing a cost function. The
% function is encoded in the file CostMatrix.m which make a matrix that
...
% so that it also considers intensities in matching cells (i.e. it prefers
% to match cells with similar intensities). Note you may need to optimize
